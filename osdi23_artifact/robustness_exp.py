...
from robustness_suite import azure_v1_suite, azure_v2_suite
...
if __name__ == "__main__":
    ...
    if args.workload == "azure_v1":
        policies = ["sr-greedy", "sr-replace-120", "mp-search"]
    else:
    ...
    # Randomize start and end time for train and test
    train_sets = []
    test_sets = []
    n_exp = 3
    np.random.seed(1)
    if args.workload == "azure_v1":
        # random day
        train_start_days = np.random.randint(0, 14, n_exp)
        train_start_hours = np.random.randint(0, 24, n_exp)
        test_start_days = np.random.randint(0, 14, n_exp)
        test_start_hours = np.random.randint(0, 24, n_exp)
        for i in range(n_exp):
            train_start = f"{str(train_start_days[i])}.{train_start_hours[i]}.0"
            if train_start_hours[i] < 23:
                train_end = f"{str(train_start_days[i])}.{train_start_hours[i] + 1}.0"
            else:
                train_end = f"{str(train_start_days[i])}.{train_start_hours[i]}.60"

            test_start = f"{str(test_start_days[i])}.{test_start_hours[i]}.0"
            if test_start_hours[i] < 23:
                test_end = f"{str(test_start_days[i])}.{test_start_hours[i] + 1}.0"
            else:
                test_end = f"{str(test_start_days[i])}.{test_start_hours[i]}.60"
            train_sets.append((train_start, train_end))
            test_sets.append((test_start, test_end))
    else:
        train_start_days = np.random.randint(0, 14, n_exp)
        test_start_days = np.random.randint(0, 14, n_exp)
        for i in range(n_exp):
            train_start = f"{str(train_start_days[i])}.0.0"
            if train_start_days[i] < 13:
                train_end = f"{str(train_start_days[i] + 1)}.0.0"
            else:
                train_end = "13.23.60"
            test_start = f"{str(test_start_days[i])}.0.0"
            if test_start_days[i] < 13:
                test_end = f"{str(test_start_days[i] + 1)}.0.0"
            else:
                test_end = "13.23.60"
            train_sets.append((train_start, train_end))
            test_sets.append((test_start, test_end))

    for experiment_i in range(n_exp):

        train_start, train_end = train_sets[experiment_i]
        test_start, test_end = test_sets[experiment_i]
        print(f"=== Train on {train_start} - {train_end}, test on {test_start} - {test_end} ===")
        file_suffix = f"train-{train_start}-{train_end}_test-{test_start}-{test_end}"
        ...
        for i in range(M):
            ...
            run_equal_model_cases(cases[start_case:end_case],
                                  output_file=output_file[:-3] + file_suffix + ".tsv",
