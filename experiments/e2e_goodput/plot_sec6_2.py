...
def plot_goodput_vs_num_devices(azurev1_lines, azurev2_lines, threshold, folder, pdf, ybottom, plot_legend=False):
    ...
    for data, lines in zip(azurev1_data, azurev1_lines):
        for line in lines:
            if line["exp_name"] != "goodput_vs_num_devices" or \
               line["policy_name"] == "sr-replace-30":
                continue
...
def plot_goodput_vs_num_models(azurev1_lines, azurev2_lines, threshold, folder, pdf, ybottom, plot_legend=False):
    ...
    for data, lines in zip(azurev1_data, azurev1_lines):
        for line in lines:
            if line["exp_name"] != "goodput_vs_num_models" or \
               line["policy_name"] == "sr-replace-30":
                continue
...
def plot_goodput_vs_slo(azurev1_lines, azurev2_lines, threshold, folder, pdf, ybottom, plot_legend=False):
    ...
    for data, lines in zip(azurev1_data, azurev1_lines):
        for line in lines:
            if line["exp_name"] != "goodput_vs_slo" or \
               line["policy_name"] == "sr-replace-30":
                continue
...
def plot_goodput_vs_rate_scale(azurev1_lines, azurev2_lines, threshold, folder, pdf, ybottom, plot_legend=False):
    ...
    for data, lines in zip(azurev1_data, azurev1_lines):
        for line in lines:
            if line["exp_name"] != "goodput_vs_rate_scale" or \
               line["policy_name"] == "sr-replace-30":
                continue
...
def plot_goodput_vs_cv_scale(azurev1_lines, azurev2_lines, threshold, folder, pdf, ybottom, plot_legend=False):
    ...
    for data, lines in zip(azurev1_data, azurev1_lines):
        for line in lines:
            if line["exp_name"] != "goodput_vs_cv_scale" or \
               line["policy_name"] == "sr-replace-30":
                continue
...
if __name__ == "__main__":
    ...
    bert1dot3_azurev1 = args.input + "/azure_v1_1dot3.tsv"
    bert6dot7_azurev1 = args.input + "/azure_v1_6dot7.tsv"
    mixed_azurev1 = args.input + "/azure_v1_mixed.tsv"

    bert1dot3_azurev2 = args.input + "/azure_v2_1dot3.tsv"
    bert6dot7_azurev2 = args.input + "/azure_v2_6dot7.tsv"
    ...
    plot_goodput_vs_num_models(azure_v1_lines, azure_v2_lines, threshold, args.output_dir, args.pdf, 60)
